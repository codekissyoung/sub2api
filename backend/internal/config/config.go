...
// 票的主要来源是被动捕获：真实业务响应头里上游铸造的 blob 直接落存，零额外流量。
// 主动打票（harvester）仅在 inject 开启时运行，走 harvest_proxy_url（SOCKS），
// 业务出站仍用账号住宅 proxy_id，只替换该请求头。
...
type OpenAICodexTicketConfig struct {
	...
	// Inject 控制是否把打到的票注入业务请求并参与调度门控。
	// 默认 true；enabled=true 且 inject=false 为观察模式：仅后台打票和展示，不影响转发链路。
...
func setDefaults() {
	...
	viper.SetDefault("gateway.openai_codex_ticket.inject", true)
	...
	// 2026-09-20 事故：默认 true 会在 inject 开启时把无票账号逐出门控模型
	// （astra/sol）调度——9 个号瞬间只剩 4 个能接 sol。门票语义永远是
	// fail-open：无票裸发、从响应里学新票；fail_closed 仅为显式 yaml 保留。
...
func (c *Config) Validate() error {
	...
	if c.Gateway.ResponsesNonstreamKeepaliveInterval < 0 {
		return fmt.Errorf("gateway.responses_nonstream_keepalive_interval must be non-negative")
	}
	if c.Gateway.ResponsesNonstreamKeepaliveInterval != 0 &&
