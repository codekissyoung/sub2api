export default {
    settings: {
      ...
      rateLimit429Cooldown: {
        ...
        adaptiveEnabled: '启用分级冷却',
        adaptiveEnabledHint: 'OpenAI 泛化 429 按连续次数使用共享 Redis 协调的分级冷却；成功一次即清零',
        cooldownSeconds: '回避时长（秒）',
        fallbackCooldownSeconds: 'Redis 异常时的固定回避（秒）',
        cooldownSecondsHint: '默认回避持续时间（1-7200 秒）；上游返回明确 reset 时仍优先使用上游时间',
        firstCooldownSeconds: '首次（秒）',
        secondCooldownSeconds: '第二次（秒）',
        thirdCooldownSeconds: '第三次及以后（秒）',
        strikeWindowSeconds: '连续统计窗口（秒）',
        adaptiveHint: '同一冷却窗口内的并发 429 会合并为一次，不会瞬间升级；明确的上游 reset 时间始终优先。',
